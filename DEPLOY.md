...
docker compose up -d
```
The app now serves on port 80.

### 8. Verify it's up
```bash
docker compose ps               # app should be "running", "0.0.0.0:80->8000"
docker compose logs app         # no errors; should not say "not provisioned"
curl -s -o /dev/null -w '%{http_code}\n' http://localhost/dashboard.html   # 200
```
From another device on the LAN: open `http://<vm-ip>/` (it redirects to the dashboard).
...
1. Open `http://<vm-ip>/teller.html`, log in with the staff password.
2. Click **Start event**.
...
- Members: `http://<vm-ip>/member.html`
- Staff teller: `http://<vm-ip>/teller.html`
- Public dashboard (prices, news, leaderboard feed): `http://<vm-ip>/dashboard.html`
- Bare `http://<vm-ip>/` redirects to the dashboard.

**Export final results** (CSV of every member's net worth): logged in as staff in the browser, open `http://<vm-ip>/api/export` — it downloads `paper-market.csv`.
...
| Symptom | Cause / fix |
|---|---|
| `http://localhost/` refused | Port 80 already in use, or container not up. `docker compose ps` / `logs app`. Remap host port in `docker-compose.yml` (`"8080:8000"`) → use `http://localhost:8080/`. |
...
| Can't reach from another device | Firewall/security group not allowing inbound TCP 80; or devices on a different subnet / guest Wi-Fi with AP isolation. |
