...
## Local Docker test
The app serves the API + frontend directly on port 80 (no proxy, plain HTTP).
...
4. Open `http://localhost/` (redirects to the dashboard) or `http://localhost/member.html` / `/teller.html`. From another LAN device use `http://<machine-ip>/…`. Staff-login on `/teller.html` and click **Start event** to unfreeze the market.
...
6. Start the stack: `docker compose up -d`. The app serves both the API and the static frontend on port 80 — plain HTTP, no reverse proxy, no TLS.
7. Print `config/pins.csv` as member cards and announce PINs.
8. **At kickoff:** open `http://<vm-ip>/teller.html` and click **Start event** — this starts the clock; the market unfreezes and trading opens. (A crash/restart resumes from this start time; it never resets.)
